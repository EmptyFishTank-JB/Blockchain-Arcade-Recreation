...
| VS | VS CPU (`cpu.js`): you against an EASY / NORMAL / HARD computer opponent (faster and smarter up the levels), on Normal rules with no exploits and the same bits in the same order. Every 30 points a drop scores sends one encrypted block (a one-peel layer hiding a random bit) onto the other board, dropping onto the top of random columns after its next move; your chains cancel blocks headed your way first. The first to overflow loses. The CPU moves on its own clock from your first drop (paused while a panel is open). LAYERS: ON / OFF turns the usual layer row every 8 drops on or off for both boards. The CPU's board shows in miniature in the HUD, and the header slims down to make room. RECORDS → STATS keeps wins and losses per level |
